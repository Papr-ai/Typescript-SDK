...
import { Document, DocumentUploadParams, DocumentUploadResponse } from './resources/document';
import {
  AddMemory,
  AddMemoryResponse,
  ContextItem,
...
import { Search, SearchSearchParams } from './resources/search';
import {
  User,
  UserCreateParams,
  UserDeleteResponse,
...
export interface ClientOptions {
  /**
   * Defaults to process.env['PAPR_PYTHON_SDK_API_KEY'].
   */
  apiKey?: string | undefined;

  /**
   * Defaults to process.env['PAPR_PYTHON_SDK_BEARER_TOKEN'].
   */
...
export class Papr {
   ...
   * API Client for interfacing with the Papr API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['PAPR_PYTHON_SDK_API_KEY'] ?? undefined]
   * @param {string | undefined} [opts.bearerToken=process.env['PAPR_PYTHON_SDK_BEARER_TOKEN'] ?? undefined]
  ...
  constructor({
    ...
    apiKey = readEnv('PAPR_PYTHON_SDK_API_KEY'),
    bearerToken = readEnv('PAPR_PYTHON_SDK_BEARER_TOKEN'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.PaprError(
        "The PAPR_PYTHON_SDK_API_KEY environment variable is missing or empty; either provide it, or instantiate the Papr client with an apiKey option, like new Papr({ apiKey: 'My API Key' }).",
      );
    }
    if (bearerToken === undefined) {
      throw new Errors.PaprError(
        "The PAPR_PYTHON_SDK_BEARER_TOKEN environment variable is missing or empty; either provide it, or instantiate the Papr client with an bearerToken option, like new Papr({ bearerToken: 'My Bearer Token' }).",
      );
    }
...
export declare namespace Papr {
  ...
  export { Search as Search, type SearchSearchParams as SearchSearchParams };
}
